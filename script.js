document.addEventListener('DOMContentLoaded', () => {
    ...
    const bsAddPromoModal = document.getElementById('addPromotionModal');
    const addPromoModal = bsAddPromoModal ? new bootstrap.Modal(bsAddPromoModal) : null;
    ...
    function populateFloors() {
        ...
        if (currentBuildingKey === 'all') {
            for (const bldg of Object.values(bldgs)) {
                if (bldg.floors > maxFloors) maxFloors = bldg.floors;
            }
        } else {
            maxFloors = bldgs[currentBuildingKey].floors;
        }

        for (let f = 1; f <= maxFloors; f++) {
            const option = document.createElement('option');
            option.value = f.toString();
            option.textContent = `ชั้น ${f} (Fl ${f})`;
    ...
    function populateBuildings() {
        ...
        const allOption = document.createElement('option');
        allOption.value = 'all';
        allOption.innerHTML = '<span class="text-th">ทุกอาคาร</span><span class="text-en">All Buildings</span>';
    ...
    function renderMatrix() {
        ...
        currentStats = { total: 0, avail: 0, booked: 0, locked: 0, processing: 0 };

        const bldgs = estateData[currentProjectKey].buildings;

        if (currentBuildingKey === 'all') {
            for (const [bKey, data] of Object.entries(bldgs)) {
                renderBuilding(bKey, data);
            }
        } else {
            const data = bldgs[currentBuildingKey];
            renderBuilding(currentBuildingKey, data);
        }
    ...
    function renderBuilding(bKey, data) {
        const bldgHeader = document.createElement('div');
        bldgHeader.className = 'w-100 mt-3 mb-2 fw-bold text-primary border-bottom pb-1 font-thai fs-6';
        bldgHeader.textContent = data.label;
        matrixContainer.appendChild(bldgHeader);

        for (let f = data.floors; f >= 1; f--) {
            ...
            floorRow.dataset.floor = f.toString();

            const floorLabel = document.createElement('div');
            floorLabel.className = 'unit-row-label me-2 font-thai small d-flex flex-column align-items-center justify-content-center gap-1';
            // Bilingual floor label with lock and unlock buttons
            floorLabel.innerHTML = `
                <div><span class="text-th">ชั้น ${f}</span><span class="text-en">Fl ${f}</span></div>
                <div class="d-flex gap-1">
                    <button class="btn btn-sm btn-outline-secondary p-0 d-flex align-items-center justify-content-center" style="width: 24px; height: 24px;" title="Lock Floor" onclick="lockFloor('${bKey}', ${f}, this)">
                        <i class="bi bi-lock-fill" style="font-size: 0.75rem;"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-success p-0 d-flex align-items-center justify-content-center" style="width: 24px; height: 24px;" title="Unlock Floor" onclick="unlockFloor('${bKey}', ${f}, this)">
            ...
            for (let u = 1; u <= data.unitsPerFloor; u++) {
                ...
                const unitNo = `${bKey}${f}${u.toString().padStart(2, '0')}`;
                ...
                if (rand > 0.85) status = 'locked';
                else if (rand > 0.65) status = 'booked';

                // Random Location Logic
                let location = 'general';
                const locRand = Math.random();
                if (locRand > 0.9) location = 'pool';
                else if (locRand > 0.8) location = 'river';
                else if (locRand > 0.7) location = 'elevator';
                else if (locRand > 0.6) location = 'stairs';
                ...
                unitBox.dataset.location = location;
    ...
    function updateDashboardStats() {
        ...
        const statProc = document.getElementById('stat-processing');
        if (statProc) statProc.textContent = currentStats.processing;
    }
    ...
    if (bsModalElement) {
        bsModalElement.addEventListener('hidden.bs.modal', () => {
            if (selectedUnitBoxRef && selectedUnitBoxRef.dataset.status === 'processing') {
                const origStatus = selectedUnitBoxRef.dataset.originalStatus || 'avail';
                selectedUnitBoxRef.dataset.status = origStatus;
                selectedUnitBoxRef.className = `flex-shrink-0 unit-box status-${origStatus}`;
                currentStats.processing--;
                currentStats[origStatus]++;
                updateDashboardStats();
                applyCurrentFilter();
            }
        });
    ...
    function openQuotationModal(unitNo, floor, price, unitType, elementBox) {
        const locationVal = elementBox.dataset.location || 'general';
        let locationText = 'ทั่วไป (General)';
        if (locationVal === 'stairs') locationText = 'ติดบันได (Stairs)';
        else if (locationVal === 'elevator') locationText = 'ติดลิฟต์ (Elevator)';
        else if (locationVal === 'river') locationText = 'ติดแม่น้ำ (River)';
        else if (locationVal === 'pool') locationText = 'ติดสระว่ายน้ำ (Pool)';
        ...
        if (modalLoc) modalLoc.textContent = locationText;
        
        const docLoc = document.getElementById('doc-location');
        if (docLoc) docLoc.textContent = locationText;

        currentBasePrice = price;
        currentTotalDiscount = 0;
        customPromotions = [
            { name: '<span class="text-th">ฟรี ค่าธรรมเนียมการโอนกรรมสิทธิ์</span><span class="text-en">Free Transfer Fee</span>', amount: 0 },
            { name: '<span class="text-th">ฟรี ค่าส่วนกลาง 1 ปีล่วงหน้า</span><span class="text-en">Free 1-year CAM Fee</span>', amount: 0 },
            { name: '<span class="text-th">ระบบ Home Automation ครบชุด</span><span class="text-en">Smart Home Automation</span>', amount: 100000 }
        ]; // Reset to default promotions

        // Calculate total discount from default promotions
        customPromotions.forEach(p => { currentTotalDiscount += p.amount; });

        renderPromotions();

        const reservationFeeInput = document.getElementById('input-reservation-fee');
        if (reservationFeeInput) reservationFeeInput.value = 50000;
        
        const contractFeeInput = document.getElementById('input-contract-fee');
        if (contractFeeInput) contractFeeInput.value = 150000;
        ...
        if (selectedUnitBoxRef && selectedUnitBoxRef.dataset.status === 'avail') {
            selectedUnitBoxRef.dataset.originalStatus = 'avail';
            selectedUnitBoxRef.dataset.status = 'processing';
            selectedUnitBoxRef.className = 'flex-shrink-0 unit-box status-processing';
            currentStats.avail--;
            currentStats.processing++;
            updateDashboardStats();
            applyCurrentFilter();
    ...
    function renderPromotions() {
        ...
        customPromotions.forEach((promo, index) => {
            const li = document.createElement('li');
            li.className = 'mb-1';

            let amountHtml = '';
            if (promo.amount > 0) {
                amountHtml = `<span class="text-danger fw-bold ms-2">- ฿ ${parseInt(promo.amount).toLocaleString()}</span>`;
            }

            li.innerHTML = `
                <div class="d-inline-flex justify-content-between align-items-start w-100">
                    <div>
                        <span>${promo.name}</span>${amountHtml}
                    </div>
                    <button type="button" class="btn btn-sm btn-link text-danger p-0 ms-2" onclick="removeCustomPromotion(${index})" title="ลบ promotion">
        ...
        const netPrice = currentBasePrice - currentTotalDiscount;
        document.getElementById('calc-promo').textContent = `- ฿ ${currentTotalDiscount.toLocaleString()}`;
        document.getElementById('calc-net').textContent = `฿ ${netPrice.toLocaleString()}`;
    }

    window.removeCustomPromotion = function (index) {
        const removed = customPromotions.splice(index, 1)[0];
        currentTotalDiscount -= parseInt(removed.amount || 0);
        renderPromotions();
    };

    // Promotion Modal Logic
    const btnOpenPromoModal = document.getElementById('btn-open-promo-modal');
    if (btnOpenPromoModal && addPromoModal) {
    ...
    // List of known autocomplete promotions
    const knownPromotions = new Set([
        "ฟรี ค่าธรรมเนียมการโอนกรรมสิทธิ์",
        "ฟรี ค่าส่วนกลาง 1 ปีล่วงหน้า",
        "ระบบ Home Automation ครบชุด"
    ]);
    ...
    if (btnSavePromotion) {
        btnSavePromotion.addEventListener('click', () => {
            ...
            let amount = parseInt(document.getElementById('promoAmount').value);

            if (!name) {
                alert(document.body.classList.contains('lang-en')
                    ? "Please enter a promotion name."
                    : "กรุณากรอกชื่อ promotion");
                return;
            }
            ...
            // Save to datalist if it's a new promotion
            if (!knownPromotions.has(name)) {
                knownPromotions.add(name);
                const datalist = document.getElementById('promoNameList');
                if (datalist) {
                    const option = document.createElement('option');
                    option.value = name;
            ...
            customPromotions.push({ name, amount });
            currentTotalDiscount += amount;
            renderPromotions();
            addPromoModal.hide();
        });
    }
    ...
    window.lockUnit = function (unitBox, unitNo) {
        const prevStatus = unitBox.dataset.status;
        ...
        // Update Stats
        if (currentStats[prevStatus] !== undefined) {
            currentStats[prevStatus]--;
        } else {
            currentStats.avail--;
        }
        currentStats.locked++;
    ...
    window.lockFloor = function (bKey, floor, btnEl) {
        const confirmMsg = document.body.classList.contains('lang-en') 
            ? `Do you want to lock all available units on Floor ${floor} of Building ${bKey}?`
            : `คุณต้องการล็อคห้องทั้งหมดในอาคาร ${bKey} ชั้น ${floor} ที่ยังว่างอยู่ ใช่หรือไม่?`;
            
        if (!confirm(confirmMsg)) return;

        // Find the parent floor row
        const floorRow = btnEl.closest('.floor-row');
        if (!floorRow) return;

        const units = floorRow.querySelectorAll('.unit-box');
        let lockedCount = 0;

        units.forEach(unitBox => {
            const status = unitBox.dataset.status;
            // Only lock available units (not booked or locked)
            if (status === 'avail' || status === 'processing') {
                const unitNo = unitBox.dataset.unit;
                lockUnit(unitBox, unitNo);
                lockedCount++;
            }
        });

        if (lockedCount > 0) {
            const successMsg = document.body.classList.contains('lang-en')
                ? `Successfully locked ${lockedCount} units.`
                : `ล็อคห้องสำเร็จจำนวน ${lockedCount} ห้อง`;
            alert(successMsg);
        } else {
            const noneMsg = document.body.classList.contains('lang-en')
                ? `No available units to lock on this floor.`
                : `ไม่มีห้องว่างในชั้นนี้ที่สามารถล็อคได้`;
            alert(noneMsg);
        }
    };

    window.unlockFloor = function (bKey, floor, btnEl) {
        const confirmMsg = document.body.classList.contains('lang-en') 
            ? `Do you want to unlock all locked units on Floor ${floor} of Building ${bKey}?`
            : `คุณต้องการปลดล็อคห้องทั้งหมดในอาคาร ${bKey} ชั้น ${floor} ใช่หรือไม่?`;
        ...
        units.forEach(unitBox => {
            ...
            if (status === 'locked') {
                ...
                // Directly unlock without prompt (bulk unlock)
                unitBox.className = 'flex-shrink-0 unit-box status-avail';
                unitBox.dataset.status = 'avail';
                ...
                const floorNum = parseInt(newBox.dataset.floor);
                const price = parseInt(newBox.dataset.price);
                const type = newBox.dataset.type;

                newBox.addEventListener('click', () => openQuotationModal(unitNo, floorNum, price, type, newBox));
        ...
        if (unlockedCount > 0) {
            ...
            const successMsg = document.body.classList.contains('lang-en')
                ? `Successfully unlocked ${unlockedCount} units.`
                : `ปลดล็อคห้องสำเร็จจำนวน ${unlockedCount} ห้อง`;
            alert(successMsg);
        } else {
            const noneMsg = document.body.classList.contains('lang-en')
                ? `No locked units to unlock on this floor.`
                : `ไม่มีห้องที่ถูกล็อคในชั้นนี้`;
            alert(noneMsg);
        }
    };
    ...
    btnProceedBook.addEventListener('click', () => {
        ...
        const contractText = document.getElementById('input-contract-fee').value;
        ...
        mockBookings.push({
            ...
            reservationFee: parseInt(document.getElementById('input-reservation-fee').value || 0),
        ...
        if (selectedUnitBoxRef) {
            const prevStatus = selectedUnitBoxRef.dataset.status;
    ...
    const formCustIdentify = document.getElementById('custIdentify');

    // Thai National ID Format and Validation Function
    ...
    if (btnAddCustomer && addCustModal) {
        btnAddCustomer.addEventListener('click', () => {
            if (formCustIdentify) formCustIdentify.value = '';
    ...
    if (btnSaveCustomer && customerDatalist) {
        btnSaveCustomer.addEventListener('click', () => {
            const identify = formCustIdentify ? formCustIdentify.value.replace(/-/g, '').trim() : '';
            ...
            if (identify && !validateThaiNationalID(identify)) {
                alert(document.body.classList.contains('lang-en')
                    ? "Invalid Thai National ID format."
    ...
    function applyCurrentFilter() {
        ...
        const priceFilterValue = priceSelect ? priceSelect.value : 'all';
        const floorFilterValue = floorSelect ? floorSelect.value : 'all';
        const locationFilterValue = locationSelect ? locationSelect.value : 'all';
        ...
        allUnits.forEach(unit => {
            ...
            if (locationFilterValue !== 'all') {
                if (unit.dataset.location !== locationFilterValue) {
                    locationMatch = false;
                }
            }

            const isMatch = statusMatch && priceMatch && locationMatch;
        ...
        // Hide Floors logically if no units match OR floor doesn't match
        allFloorRows.forEach(row => {
            const floorNum = row.dataset.floor;
            
            let floorMatch = true;
            if (floorFilterValue !== 'all' && floorNum !== floorFilterValue) {
                floorMatch = false;
            }

            const visibleUnitsInRow = row.querySelectorAll('.unit-box:not(.d-none)');
            if (!floorMatch || visibleUnitsInRow.length === 0) {
